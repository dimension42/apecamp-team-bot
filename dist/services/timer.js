...
exports.restoreTimer = restoreTimer;
...
function parseEndTime(timeStr) {
    ...
    if (!match)
        throw new Error('올바른 시간 형식이 아닙니다. (예: 18:00)');
    ...
    if (endUTC <= now)
        throw new Error('이미 지난 시간입니다. 올바른 종료 시간을 입력해주세요.');
    return endUTC;
}
...
async function getTeamChannels(client) {
    const { data } = await supabase_1.supabase.from('team_channel_summaries').select('channel_id');
...
async function startTimer(config) {
    const { endTime, submissionFormat, submissionMethod, client } = config;
    cancelAllTimers();
    // Supabase에 저장 (재시작 시 복구용)
    await supabase_1.supabase.from('mission_timer').upsert({
        id: 1,
    ...
    const immediateMsg = totalHours > 0
    ...
    schedule(endMs - now, async () => {
        await sendToAllTeams(client, '🚨 미션 타이머 종료! 제출 시간이 만료되었습니다. 제한 시간 내에 결과물 제출을 못 한 팀은 현장에서 직접 말씀해주시길 바랍니다.');
        await supabase_1.supabase.from('mission_timer').delete().eq('id', 1);
    });
    console.log(`✅ 미션 타이머 설정: ${formatKST(endTime)} KST`);
}
// 봇 재시작 시 타이머 복구
async function restoreTimer(client) {
    const { data } = await supabase_1.supabase.from('mission_timer').select('*').eq('id', 1).single();
    if (!data)
        return;
