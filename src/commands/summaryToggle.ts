...
import { isRegisteredTeamChannel, setSummaryEnabled } from '../services/channelMonitor'

// 자동 요약 끄기 (영문/국문)
export const summaryOffData = new SlashCommandBuilder()
  .setName('summary-off')
  .setDescription('Disable automatic summaries in this channel')

export const yoyakOffData = new SlashCommandBuilder()
  .setName('요약끄기')
  .setDescription('이 채널에서 자동 요약을 끕니다')

// 자동 요약 켜기 (영문/국문)
export const summaryOnData = new SlashCommandBuilder()
  .setName('summary-on')
  .setDescription('Re-enable automatic summaries in this channel')

export const yoyakOnData = new SlashCommandBuilder()
  .setName('요약켜기')
  .setDescription('이 채널에서 자동 요약을 다시 켭니다')

// /요약끄기·/요약켜기·/summary-off·/summary-on 공통 처리
export async function executeSummaryToggle(
  ...
  if (channel.parent?.name !== 'Team-chat' || !(await isRegisteredTeamChannel(channel.id))) {
    await interaction.reply({ content: '❌ 이 명령어는 팀 채널에서만 사용할 수 있습니다.', ephemeral: true })
    return
  }

  try {
    await setSummaryEnabled(channel.id, enabled)
  } catch (err: any) {
